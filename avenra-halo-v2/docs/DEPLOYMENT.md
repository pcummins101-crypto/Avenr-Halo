...
| `avenra_halo_v2_safety_alert_result` | Deliver test or crash alerts to the rider's nominated contact | An array with `sent` or `success` set to `true`; otherwise `null` to use Halo's built-in next-of-kin SMS (FireText or the delivery filter), then the V1 bridge |
| `avenra_halo_v2_prefer_legacy_nok_alerts` | Keep routing next-of-kin test and crash messages through the V1 admin-ajax handlers even when Halo can send SMS itself | `true` to prefer the V1 bridge; default `false` |
| `avenra_halo_v2_emergency_sms_delivery` | Replace the built-in FireText delivery for Emergency Assist responder and next-of-kin SMS (`role` is `primary`, `backup` or `next_of_kin`) | `null` to use FireText, or an array with `accepted` set to literal `true` only after provider acceptance; return a failure/unknown result otherwise |
