...
final class Avenra_Halo_V2_REST {
	...
	public function save_ride( WP_REST_Request $request ): WP_REST_Response {
		...
		$data = array(
			...
			// Measured by HyperCore BMS during the ride. Absent on a GPS-only ride.
			'energy_recovered_wh'    => $this->number( $flat['energy_recovered_wh'] ?? null, 0, 500000, null ),
			'efficiency_wh_per_mile' => $this->number( $flat['efficiency_wh_per_mile'] ?? null, 0, 100000, null ),
			'peak_power_kw'          => $this->number( $flat['peak_power_kw'] ?? null, 0, 1000, null ),
			'peak_current_a'         => $this->number( $flat['peak_current_a'] ?? null, 0, 5000, null ),
			'peak_voltage_v'         => $this->number( $flat['peak_voltage_v'] ?? null, 0, 2000, null ),
	...
	private function serialise_ride( object $row ): array {
		...
		$ride = array(
			...
			'energy_recovered_wh'  => null !== ( $row->energy_recovered_wh ?? null ) ? (float) $row->energy_recovered_wh : null,
			'efficiency_wh_per_mile' => null !== ( $row->efficiency_wh_per_mile ?? null ) ? (float) $row->efficiency_wh_per_mile : null,
			'peak_power_kw'        => null !== ( $row->peak_power_kw ?? null ) ? (float) $row->peak_power_kw : null,
			'peak_current_a'       => null !== ( $row->peak_current_a ?? null ) ? (float) $row->peak_current_a : null,
			'peak_voltage_v'       => null !== ( $row->peak_voltage_v ?? null ) ? (float) $row->peak_voltage_v : null,
